...
import { ADMIN_ACTIVITY_LIMIT, ADMIN_LIST_LIMIT, parseAdminPage } from './adminPaging';
...
export function resolveAdminMedia(url) {
...
export function mapAdminReport(raw) {
  ...
  return {
    ...
    description: raw.description || raw.message || raw.details || raw.adminNote || '',
...
export async function fetchAdminUsers({ page = 1, limit = ADMIN_LIST_LIMIT } = {}) {
...
export async function fetchAdminReports({ status = 'pending', page = 1, limit = ADMIN_LIST_LIMIT } = {}) {
...
export async function fetchPendingReportCount() {
  try {
    const res = await fetchAdminReports({ status: 'pending', page: 1, limit: ADMIN_LIST_LIMIT });
    return res.total;
  } catch {
    return 0;
  }
...
function mapAdViewers(raw = {}) {
  ...
  const image = (Array.isArray(raw.images) && raw.images[0])
...
export async function fetchAdminAdViewers({ page = 1, limit = ADMIN_LIST_LIMIT } = {}) {
...
export async function fetchAdminVisitors({ page = 1, limit = ADMIN_LIST_LIMIT } = {}) {
...
export async function fetchAdminActivityLog({ page = 1, limit = ADMIN_ACTIVITY_LIMIT, type } = {}) {
